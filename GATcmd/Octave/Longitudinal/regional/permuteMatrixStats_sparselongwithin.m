...
for k = 1:nperm
    rand("seed",1000+k);
    rndIdxT1 = randsample(size(T1,3),size(T1,3),true);
    randT1=T1(:,:,rndIdxT1);
 
    rand("seed",2000+k);
    rndIdxT2 = randsample(size(T2,3),size(T2,3),true);
    randT2=T2(:,:,rndIdxT2);
...
save(['PermStats_longwithin_' FileSaveSuffix_groupname '.mat'],'trueDiff', 'trueDiffabs', 'randDiff', 'pVal', 'pVal0', 'true_pVal0',...
    'pValnan','FDRcritP','pValFDR','pValSurv','indx','trueDiffFDR','row','-v7');

save(['TrueDiffFDR_longwithin_' FileSaveSuffix_groupname '.mat'],'trueDiffFDR','-v7');
xlswrite(['TrueDiffFDR_longwithin_' FileSaveSuffix_groupname '.xlsx'],trueDiffFDR)