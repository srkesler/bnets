...
for k = 1:nperm
    rng(1000+k)
    randG1 = datasample(G1,size(G1,3),3,'Replace',true);
    rng(2000+k)
    randG2 = datasample(G2,size(G2,3),3,'Replace',true);
...
save(['PermStats_' FileSaveSuffix_groupname],'trueDiff','trueDiffabs', 'randDiff', 'pVal', 'pVal0', 'true_pVal0',...
    'pValnan','FDRcritP','pValFDR','pValSurv','indx','trueDiffFDR','row');

save('TrueDiffFDR','trueDiffFDR')
writetable(array2table(trueDiffFDR),'TrueDiffFDR.xlsx')
