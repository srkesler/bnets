...
rng(1001); % Added by Vikram Rao on 09/17/2018 in order to fix the seed and have consistent results
...
for i=1:Nperm
    ...
    rng(1000+i); % Added by Vikram Rao on 09/17/2018 in order to fix the seed and have consistent results
    Samp1=randsample(Sz1+Sz2,Sz1,'true');
    rng(2000+i); % Added by Vikram Rao on 09/17/2018 in order to fix the seed and have consistent results
...
Mmoddegzscore_1_rand=zeros(size(N1,1),size(N1,2),size(N_G1,2));Mpartcoeff_1_rand=zeros(size(N1,1),size(N1,2),size(N_G1,2));
...
Mmoddegzscore_2_rand=zeros(size(N2,1),size(N2,2),size(N_G2,2));Mpartcoeff_2_rand=zeros(size(N2,1),size(N2,2),size(N_G2,2));
...
Mmoddegzscore_1=zeros(size(N1));Mpartcoeff_1=zeros(size(N1));
...
Mmoddegzscore_2=zeros(size(N2));Mpartcoeff_2=zeros(size(N2));
...
for i=1:size(N_G1,2)
    for j=1:size(N1,1)
        for k=1:size(N1,2)
            ...
            Mmoddegzscore_1_rand(j,k,i)=N_G1{i}{j,k}{26};
            Mpartcoeff_1_rand(j,k,i)=N_G1{i}{j,k}{28};
        end
    end
    ...
    for j=1:size(N2,1)
        for k=1:size(N2,2)
            ...
            Mmoddegzscore_2_rand(j,k,i)=N_G2{i}{j,k}{26};
            Mpartcoeff_2_rand(j,k,i)=N_G2{i}{j,k}{28};
        end
    end
end
...
for j=1:size(N1,1)
    for k=1:size(N1,2)
        ...
        Mmoddegzscore_1(j,k)=N1{j,k}{26};
        Mpartcoeff_1(j,k)=N1{j,k}{28};
    end
end
...
for j=1:size(N2,1)
    for k=1:size(N2,2)
        ...
        Mmoddegzscore_2(j,k)=N2{j,k}{26};
        Mpartcoeff_2(j,k)=N2{j,k}{28};
    end
end

fprintf('%-4s\n',' saving output...');

save NetMesPerDens Dens_1_rand MClust_1_rand MDeg_1_rand Trans_1_rand ...
    ...
    Sigma_1_rand Mmoddegzscore_1_rand Mpartcoeff_1_rand Dens_2_rand MClust_2_rand MDeg_2_rand Trans_2_rand Assort_2_rand GEff_2_rand ...
    MLocEff_2_rand Mod_2_rand ModL_2_rand PathL_2_rand MNodeBetw_2_rand ...
    MEdgeBetw_2_rand Lambda_2_rand Gamma_2_rand Sigma_2_rand Mmoddegzscore_2_rand Mpartcoeff_2_rand...
    ...
    ModL_2 PathL_2 MNodeBetw_2 MEdgeBetw_2 Lambda_2 Gamma_2 Sigma_2 Mmoddegzscore_2 Mpartcoeff_2
